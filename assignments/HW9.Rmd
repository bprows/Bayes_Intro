...
```
There is a 95% probability that the true ratio of $\sigma^2_{urban} / \sigma^2_{rural}$ is between 0.316 and 2.607. Since this interval does not exclude 0, we cannot reject the null hypothesis that the variances are equal. 
...
```{r, fig.align='center'}
J <- 10000
sim_prior <- rnorm(J, mean = mu, sd = sqrt(rinvgamma(J, shape = a, rate = b)))
sim_post <- rnorm(J, mean = mu, sd = sqrt(rinvgamma(J, shape = a_star_urban, b_star_urban)))
