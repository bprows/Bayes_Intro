...
The MLE of a binomial is $ \hat{p} = x/n $
```{r pressure, echo=FALSE, fig.cap="Insert Proof", out.width = '100%'}
knitr::include_graphics("./../pics/picture.jpg", )
```
...
```{r, echo=FALSE, fig.cap="Insert Proof", out.width = '100%'}
knitr::include_graphics("./../pics/pic.jpg")
```
...
post1 <- dbeta(X,shape1=1+x, shape2=1+n)
plot(X,post1, 
...
lines(X,prior1,
      col = 'red', lwd = 2)
legend(.4, 3.5, legend=c("Prior", "Posterior"),
       col=c("red", "blue"), lty = 1, lwd = 2, cex = 1)

prior2 <- dbeta(X,shape1=25,shape2=1)
post2 <- dbeta(X,shape1=25+x, shape2=1+n)
plot(X,prior2, 
     col = 'blue', lwd = 2, type = 'l',
